...
Công cụ CLI **flash liên tục (multi-flash)** cho nhiều thiết bị ESP32, biến thể của `Multi-ESP-Flasher` với quy trình **chọn sản phẩm phần cứng** và **ghi nhớ lựa chọn file** cho từng sản phẩm.
...
- Bản portable: nhấp đúp **`bin\MLAstro-Multi-ESP-Flasher.exe`** (không cần cài Python).
- Bản script: nhấp đúp **`MLAstro-Multi-ESP-Flasher.bat`** (cần Python 3 + `pip install esptool pyserial`).
...
- Tool **tự phát hiện** file bin có sẵn trong thư mục sản phẩm và **ghi nhớ lựa chọn** vào file **`MLAstro-Multi-ESP-Flasher.db.txt`** (đặt cạnh exe).
...
- Exe: **`bin\MLAstro-Multi-ESP-Flasher.exe`** — bundle sẵn Python + esptool + pyserial + tkinter, chạy không cần cài gì.
